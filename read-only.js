...
// Get top depositors
export async function getTopDepositors(count = 5) {
  const { contract, entriesCount } = await getContractData();
  ...
  return Array.from(depositorMap.entries())
    .map(([address, amount]) => ({
      address,
...
export async function loadJackpotInfo() {
  try {
    document.getElementById("jackpot").innerHTML = "<strong>Loading...</strong>";
    document.getElementById("usd24h").innerText = "Loading...";
    
    const { totalPool, jackpotUsd, targetUsd, last24hUsd } = await getContractData();
    
    const totalPoolEth = ethers.utils.formatEther(totalPool);
    ...
    document.getElementById("jackpot").innerHTML = 
      `<strong>${totalPoolEth} ETH</strong> ($${jackpotUsdFormatted})`;
    
    document.getElementById("usd24h").innerText = `$${last24hUsdFormatted}`;
    
    document.getElementById("progressFill").style.width = 
      `${Math.min(Number(percentComplete), 100)}%`;
    
    updateStatusMessageFromPercent(percentComplete);
    
    const topDepositors = await getTopDepositors(5);
